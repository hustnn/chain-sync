...
use super::{
    ...
    utils::{
        arrow_schema_without_partitions, next_data_path, record_batch_from_message, record_batch_without_partitions, stringified_partition_value},
    DeltaWriter, DeltaWriterError,
};
use crate::writer::utils::ShareableBuffer;
use deltalake::{action::Add, DeltaTableMetaData, Schema, PartitionFilter, DeltaTableBuilder, DeltaTable, ObjectStore, Path};
use arrow::{
...
use crate::DeltaWriterError::MissingPartitionColumn;
...
pub struct JsonWriter {
    ...
    updated_partition_values: Vec<HashMap<String, String>>,
}
...
impl JsonWriter {
    ...
    pub async fn generate_manifests(&self, table: &DeltaTable) -> Result<(), DeltaWriterError>{
        for partition in self.updated_partition_values.iter() {
            println!("Generate manifest for partition {:?}", partition);
            let mut filters :Vec<PartitionFilter<&str>> = vec![];
            let mut path_parts = vec![];
            for (k, v) in partition {
                filters.push(deltalake::PartitionFilter {
                    key: k,
                    value: deltalake::PartitionValue::Equal(v),
                });
                let part = format!("{}={}", k, v);
                path_parts.push(part);
            }
            let files = table.get_files_by_partitions(&filters).unwrap();
            let files_with_uri: Vec<String> = files.iter().map(|file| {return format!("{}/{}", table.table_uri(), file);}).collect();
            println!("Files to write {:?}", files_with_uri);
            //let path = format!("{}/{}/{}/{}", table.table_uri, "_symlink_format_manifest", path_parts.join("/"), "manifest");
            let path = Path::from(format!("{}/{}/{}", "_symlink_format_manifest", path_parts.join("/"), "manifest"));
            let res = Bytes::from(files_with_uri.join("\n").into_bytes());
            println!("Path to write {:?}", path);
            self.storage.put(&path, Bytes::from(res)).await?;
        }
        Ok(())
    }
}
...
impl DeltaWriter<Vec<Value>> for JsonWriter {
    ...
    async fn flush(&mut self) -> Result<Vec<Add>, DeltaWriterError> {
        println!("Flush # of arrow writers: {:?}", self.arrow_writers.len());
        self.updated_partition_values = collect_partition_value(&self.partition_columns, &self.arrow_writers)?;
...
fn collect_partition_value(
    partition_columns: & Vec<String>,
    arrow_writers: & HashMap<String, DataArrowWriter>) -> Result<Vec<HashMap<String, String>>, DeltaWriterError> {
    let mut partitions = vec![];
    for (key, writer) in arrow_writers {
        if !partition_columns.is_empty() {
            let mut kv = HashMap::new();
            for k in partition_columns.iter() {
                let partition_value = writer.partition_values
                    .get(k)
                ...
                kv.insert(k.to_string(), partition_value.to_string());
            }
            partitions.push(kv);
        }
    }
...
fn extract_partition_values(
    ...
    for col_name in partition_cols.iter() {
        ...
        println!("Partition string is {:?}.", partition_string);
