...
use deltalake::{DeltaDataTypeVersion, DeltaTable, DeltaTableBuilder, DeltaTableError};
...
pub(crate) async fn load_table(
    ...
    let storage = DeltaTableBuilder::from_uri(&table_uri)
        ...
        .build_storage()?;
    let mut table = DeltaTable::new(storage, deltalake::DeltaTableConfig::default());
    table.load().await?;
    Ok(table)
}
